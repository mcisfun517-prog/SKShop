            ...
            if (_pendingSells.TryGetValue(index, out var pending) && pending != null)
            {
                ...
                if (itemsChanged && moneyDelta == 0)
                {
                    ...
                    if (soldItemsNow.Count > 0)
                    {
                        ...
                        pending.IsConfigItemSell = true;
                        pending.LastTotals = new Dictionary<int, int>(curTotals);
                        return;
                    }
               
                    _skeletonItemSnapshot[index] = curTotals;
                    _skeletonMoneySnapshot[index] = curMoney;
                    _pendingSells.Remove(index);
                    _lastUsedItemTypeForSkeleton.Remove(index);
                    _lastUsedItemTimeForSkeleton.Remove(index);
                    ShopHelper.UpdateCooldown(index, cooldownMs);
                ...
                if ((DateTime.UtcNow - pending.StartTime).TotalMilliseconds > PendingTimeoutMs)
                {
                    if (!itemsChanged && moneyDelta > 0)
                    {
                        if (pending.IsConfigItemSell)
                        {
                            
                            ShopHelper.SetPlayerCopperCoins(plr, pending.LastMoney);
                            _skeletonMoneySnapshot[index] = pending.LastMoney;
                        }
                        else
                        {
